...
const AdminView = ({setApplication, setStatus, updateApplication, state, content, lang}) => {
            ...
            <form onSubmit={(e)=> { updateApplication(e)}}>
                <div className="form-group">
                    {state && state.applications && state.applicationId &&
                        <div>
                            <p>{content.adminavailability}</p>
                            {state.appData.availability.map((avail, i) => {
                                return <p key={i}>{avail.from} - {avail.to}</p>
                            })}
                            {state.appData.competence.map((comp, i) => {
                                return <p key={i}>{comp.competence[lang - 1].name}: {comp.duration} {content.admintimemetric}</p>
                            })}
                    ...
                    {state && state.applications &&
                        ...
                        <option key={0} disabled value={0}>{content.adminchooseapp}</option>
                ...
                <div className="form-group">
                    ...
                    <select id="status" name="status" defaultValue={0} onChange={ev => {setStatus(ev)}}>
                        <option key={0} disabled value={0}>{content.adminchoosestatus}</option>
                        {state.status[lang - 1].map(stat => <option key={stat.id} value={stat.id} value={stat.id}>{stat.name}</option>)}
                    </select>}
                </div>
                <div className="form-group">
                    <button type="submit" id="send">{content.adminbutton}</button>
                </div>
            </form>             
    </div>
  );
